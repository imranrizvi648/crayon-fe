...
export function LineItemsSection({ sheet }) {
  ...
  // ── YEAR TOTALS ───────────────────────────────────────────────────────────
  const sumYC = (field) =>
    items.reduce((s, i) => s + Number(i?.calculated?.[yearKey]?.[field] || 0), 0);
  const sumYF = (base) =>
    items.reduce((s, i) => s + Number(yf(i, base) || 0), 0);

  const totNet      = sumYC("total_net");
  const totErp      = sumYC("total_erp");
  const totEup      = sumYC("total_eup");
  const totQty      = sumYF("quantity");
  const totRebate   = sumYC("rebate_amount");
  const totGp       = isAfrica ? sumYC("gp")         : 0;
  const totSwoGp    = isAfrica ? sumYC("swo_gp")     : 0;
  const totPartGp   = isAfrica ? sumYC("partner_gp") : 0;

  // 3-year grand sums (for ramped footer)
  const sum3Y = (field) =>
    items.reduce((s, i) => {
      const c = i?.calculated || {};
      return s + Number(c.y1?.[field]||0) + Number(c.y2?.[field]||0) + Number(c.y3?.[field]||0);
    }, 0);

  const tot3Net = sum3Y("total_net");
  const tot3Erp = sum3Y("total_erp");
  const tot3Eup = sum3Y("total_eup");

  // GP per year from summary
    ...
    <div className="space-y-0">
      ...
      <div className="overflow-x-auto">
        <table className="w-full border-collapse text-left"
          ...
          <tbody className="divide-y divide-slate-100">
            ...
            {/* Total over 3 Years row - always show */}
            <tr className="bg-blue-900 text-white text-[11px] font-bold">
              ...
              {cols.slice(cols.findIndex(c=>c.key==="total_net")).map(c => {
                const sum3Rebate = items.reduce((s,i)=>{
                  const c2=i?.calculated||{};
                  return s+Number(c2.y1?.rebate_amount||0)+Number(c2.y2?.rebate_amount||0)+Number(c2.y3?.rebate_amount||0);
                },0);
                const v3 = {
                  total_net:  fmtAED(tot3Net,    currency),
                  total_erp:  fmtAED(tot3Erp,    currency),
                  total_eup:  fmtAED(tot3Eup,    currency),
                  rebate_amt: fmtAED(sum3Rebate,  currency),
                  qty:        "",
                ...
                return (
                  <td key={c.key} className="px-2 py-2 border-r border-blue-800 whitespace-nowrap text-right">
                    {v3[c.key] || ""}
                  </td>
                );
              })}
      ...
      <div className="flex justify-end p-6 bg-[#F8FAFC] border-t border-slate-200">
        <div className="w-[320px] space-y-1.5">
          ...
          {/* Profit per year */}
          {[
            { label: "Profit /Year 1",  val: csum.gp?.y1 },
            { label: "Profit /Year 2",  val: csum.gp?.y2 },
            { label: "Profit /Year 3",  val: csum.gp?.y3 },
          ].map(r => (
