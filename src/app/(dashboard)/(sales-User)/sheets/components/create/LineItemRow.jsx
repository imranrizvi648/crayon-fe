...
export function calcLineItem(item, exchangeRate = 3.6725, dealType = "NORMAL", yearSuffix = "") {
  const s = yearSuffix;
  ...
  const msDiscPct   = Number(item[`ms_discount${s}`]   ?? item.ms_discount   ?? 0) / 100;
  const crayonMuPct = Number(item[`crayon_markup${s}`] ?? item.crayon_markup ?? 0) / 100;
  const rebatePct   = Number(item[`rebate_percent${s}`]?? item.rebate_percent?? 0) / 100;
  const qty         = Number(item[`qty${s}`]           ?? item.qty           ?? 0);
  const unitType    = Number(item.unit_type ?? 12);
  // FIX: swo_gp_percent is PER YEAR → swo_gp_percent | swo_gp_percent_y2 | swo_gp_percent_y3
  const swoGpPct    = Number(item[`swo_gp_percent${s}`] ?? item.swo_gp_percent ?? 50) / 100;
  ...
  const calcMarkupPct = msDiscNet > 0 ? ((eupUnit - msDiscNet) / msDiscNet) * 100 : 0;

  return { msDiscNet, msDiscErp, totalNet, totalErp, eupUnit, totalEup, rebateAmt, gp, swoGp, partnerGp, defaultMarkup, calcMarkupPct };
}

// ─── ROW COMPONENT ────────────────────────────────────────────────────────────

export function LineItemRow({
  item, index, onUpdate, onRemove, region, onBatchPaste,
  activeYear, exchangeRate = 3.6725, dealType = "NORMAL"
}) {
  ...
  const handlePaste = (e) => {
    ...
    for (const line of joined) {
      ...
      const swoVal = pctToPlain(cols[19]) || 50;
      ...
      const newItem = {
        ...
        part_number: partNo,
        item_name:   name,
        unit_type:   parseInt(pn(cols[7])) || 12,
  ...
  return (
    <TableRow className="hover:bg-blue-50/20 border-b border-slate-200">
      ...
      {/* Category */}
      <TableCell className="min-w-[170px] px-1 py-1">
        <Select value={item.category || "Enterprise Online"} onValueChange={v => onUpdate("category", v)}>
          <SelectTrigger className="h-9 text-[11px] w-full"><SelectValue /></SelectTrigger>
          <SelectContent>
            <SelectItem value="Enterprise Online">Enterprise Online</SelectItem>
            <SelectItem value="Additional">Additional</SelectItem>
            <SelectItem value="Additional On Premise">Additional On Premise</SelectItem>
          </SelectContent>
        </Select>
      </TableCell>

      {/* Part Number – paste target */}
      <TableCell className="min-w-[140px] px-1 py-1 bg-green-50/40">
        <Input className={`${inputCls} border-green-300`}
      ...
      {/* Item Name */}
      <TableCell className="min-w-[240px] px-1 py-1">
      ...
      {/* Unit Net USD */}
      <TableCell className="min-w-[100px] px-1 py-1">
        <Input className={numInputCls} type="number" step="0.01"
          value={yv("unit_net_usd")}
          onChange={e => onUpdate(yf("unit_net_usd"), parseFloat(e.target.value) || 0)}
        />
      </TableCell>
      ...
      <TableCell className="min-w-[100px] px-1 py-1">
        <Input className={numInputCls} type="number" step="0.01"
          ...
          onChange={e => onUpdate(yf("unit_erp_usd"), parseFloat(e.target.value) || 0)}
        />
      </TableCell>

      {/* Default Markup % – read-only */}
      <TableCell className={`min-w-[80px] ${calcCell} bg-slate-50 text-slate-500`}>
      ...
      <TableCell className="min-w-[90px] px-1 py-1">
        <Input className={numInputCls} type="number" step="0.01"
          ...
          onChange={e => onUpdate(yf("ms_discount"), parseFloat(e.target.value) || 0)}
        />
      </TableCell>

      {/* Crayon Markup % */}
      <TableCell className="min-w-[90px] px-1 py-1">
        <Input className={numInputCls} type="number" step="0.001"
          value={yv("crayon_markup")}
          onChange={e => onUpdate(yf("crayon_markup"), parseFloat(e.target.value) || 0)}
        />
      </TableCell>

      {/* Unit Type */}
      <TableCell className="min-w-[75px] px-1 py-1">
        <Input
          className="h-9 text-[11px] text-center w-full border-slate-300"
          inputMode="numeric"
          value={item.unit_type === undefined || item.unit_type === null ? 12 : item.unit_type}
          onChange={e => {
      ...
      {/* MS Disc Net – calc */}
      <TableCell className={`min-w-[95px] ${calcCell} bg-blue-50/40 text-blue-700`}>
        {calc.msDiscNet.toFixed(3)}
      </TableCell>

      {/* MS Disc ERP – calc */}
      <TableCell className={`min-w-[95px] ${calcCell} bg-blue-50/40 text-blue-700 border-r`}>
        {calc.msDiscErp.toFixed(2)}
      </TableCell>

      {/* Total Net – calc */}
      <TableCell className={`min-w-[130px] ${calcCell} bg-blue-100/60 font-bold text-slate-800`}>
        {fmtAED(calc.totalNet)}
      </TableCell>

      {/* Total ERP – calc */}
      <TableCell className={`min-w-[130px] ${calcCell} bg-blue-100/60 font-bold text-slate-800 border-r`}>
        {fmtAED(calc.totalErp)}
      </TableCell>

      {/* Qty */}
      <TableCell className="min-w-[80px] px-1 py-1 bg-yellow-50/60">
        <Input className="h-9 text-[11px] text-center font-bold w-full border-yellow-300"
          inputMode="numeric"
          value={yv("qty")}
      ...
      {/* EUP unit – calc */}
      <TableCell className={`min-w-[95px] ${calcCell} text-green-700`}>
        {calc.eupUnit.toFixed(2)}
      </TableCell>

      {/* Total EUP – calc */}
      <TableCell className={`min-w-[130px] ${calcCell} bg-green-100/60 font-bold text-green-900 border-x`}>
      ...
      {/* Rebate % */}
      <TableCell className="min-w-[85px] px-1 py-1 border-l">
        <Input className={numInputCls} type="number" step="0.01"
          value={yv("rebate_percent")}
          onChange={e => onUpdate(yf("rebate_percent"), parseFloat(e.target.value) || 0)}
        />
      </TableCell>

      {/* Rebate Amount – calc */}
      <TableCell className={`min-w-[120px] ${calcCell} bg-yellow-50/60 text-yellow-800`}>
      ...
      {isAfrica && (
        <>
          <TableCell className={`min-w-[120px] ${calcCell} bg-purple-50/50 text-purple-800`}>
            {fmtAED(calc.gp)}
          </TableCell>
          {/* FIX: year-aware via yv/yf */}
          <TableCell className="min-w-[80px] px-1 py-1 bg-purple-100/50">
            <Input className="h-9 text-[11px] text-center w-full" type="number"
              value={yv("swo_gp_percent")}
              onChange={e => onUpdate(yf("swo_gp_percent"), parseFloat(e.target.value) || 0)}
            />
          </TableCell>
          <TableCell className={`min-w-[120px] ${calcCell} bg-purple-50/50 text-purple-800`}>
            {fmtAED(calc.swoGp)}
          </TableCell>
          <TableCell className={`min-w-[120px] ${calcCell} bg-orange-50/50 text-orange-800`}>
